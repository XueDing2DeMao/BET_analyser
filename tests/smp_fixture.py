...
def conditions(n_ads, p0, *, coefficients):
...
def measurement(source, *, mass, coefficients):
    ...
    data = b'\x0d\xe0\x03\x00\x09\x01\x00\x01\x00'
    ...
    for i, (pressure, relative, dose) in enumerate(zip(p, values[:, 0], doses), 1):
        previous = values[i-2, 0] if i > 1 else 0.
        data += record((pressure, relative, dose), minutes=i, previous=previous)
    return data + conditions(len(source['ads']), np.full(len(p)+1, 760.), coefficients=coefficients)
...
def make_smp(*, mass=.5, name='Synthetic', padding=0, coefficients=(.002, .05)):
    ...
    sections[303] = measurement(source, mass=mass, coefficients=coefficients)
